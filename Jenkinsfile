node {
	...
	stage('Deploy to tomcat'){
		...
		echo "${env.PROJ_DIR}"
		sshPublisher failOnError: true, publishers: [sshPublisherDesc(configName: 'ansible_controller', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: "echo $PROJ_DIR;cd $PROJ_DIR;ansible-playbook -i inventory.txt site.yml -K -e \'{\"war_file_path\":\"$PROJ_DIR\",\"war_file_name\":\"\$(ls $PROJ_DIR | head -n 1)\"}\';echo $BUILD_NUMBER;hostname", execTimeout: 120000, flatten: true, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: env.PROJ_DIR, remoteDirectorySDF: false, removePrefix: '', sourceFiles: 'target/**/*.war,site.yml,inventory.txt')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)]
		}
	}
}